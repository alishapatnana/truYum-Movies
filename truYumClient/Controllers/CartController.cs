    ...
    public class CartController : Controller
    {
        ...
        public async Task<ActionResult> AddToCartAsync(int id)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                using (var response = await client.GetAsync("https://localhost:44362/api/Movies/" + id))
                {
                ...
                using (var response = await client.PostAsync("https://localhost:44321/api/Cart/" + userid, content))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                    menuItem = JsonConvert.DeserializeObject<Movies>(apiResponse);
                }
        ...
        public async Task<ActionResult> GetCartItemsAsync()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                using (var response = await client.GetAsync("https://localhost:44321/api/Cart/" + id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ActionResult> PlaceOrderAsync()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                client.DefaultRequestHeaders.Authorization =
                    new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("token"));
                var userid = HttpContext.Session.GetInt32("Userid");
                using (var response = await client.GetAsync("https://localhost:44321/api/Cart/user?userid=" + userid))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ActionResult> DeleteItem(int id, int user)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                using (var response = await client.DeleteAsync("https://localhost:44321/api/Cart/" + userid + "?menuitemid=" + id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
